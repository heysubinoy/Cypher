...
function ChatThreadList({
  ...
  const { activeComposerState, activeThread, composerStates, socketUrl } =
    useComposerContext();
  ...
  useEffect(() => {
    ...
    // Add a reusable function for socket communication
    ...
    if (
      ...
      sendToExternalService(socketUrl, {
        type: "response",
  ...
  const state = useMemo(() => {
    ...
    return activeThread.id === activeComposerState.threadId
      ? activeComposerState
      : composerStates.find((s) => s.threadId === activeThread.id);
