...
export default function Compose() {
  ...
  const showWelcomeScreen =
    !initialized || !activeComposerState?.messages.length;
            ...
            {initialized && (
              <>
                <span className="text-[var(--vscode-input-foreground)] leading-relaxed">
                  Start exploring your codebase, ask questions about your
                  project, or get AI-assisted coding help.
                  <br />
                  <br />
                  One line ahead of every error.
                </span>
                <div className="inline-block mt-6 px-4 py-2 rounded-lg bg-slate-700/20 border border-slate-700/40">
                  <section className="flex flex-col items-center gap-2 text-sm">
                    ...
                    <div>
                      Type{" "}
                      <kbd className="px-2 py-0.5 rounded bg-slate-700/30">
                        @
                      </kbd>{" "}
                      to reference a file directly, or highlight text in your
                      editor
                    </div>
                  </section>
                </div>
  ...
  return (
    <>
      ...
      <main className="flex-1 min-h-0 flex flex-col text-base">
        <ErrorBoundary
          resetKeys={[activeComposerState ?? 0]}
          fallback={
            <div className="flex items-center justify-center h-full p-4 bg-[var(--vscode-input-background)] rounded-md">
              <div className="text-center max-w-lg p-6">
                ...
                <p className="mb-4">
                  We couldn't load your messages. Please try clearing your chat
                  history.
                </p>
              </div>
            </div>
          ...
          <div className="flex-shrink-0 mb-2">
            {displayTokens && (
              <div className="mt-4 flex justify-center items-center gap-3 text-gray-400/70 text-sm">
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${cssClasses}`}
                >
                  <span className="font-medium text-xs uppercase tracking-wider">
                    Usage
                  </span>
                  <span className="flex items-center text-blue-400/90">
                    <VscArrowUp className="mr-1" size={12} />{" "}
                    {inputTokens.toLocaleString()}
                  </span>
                  <span className="flex items-center text-green-400/90">
                    <VscArrowDown className="mr-1" size={12} />{" "}
