...
const ChatInput = ({
  ...
  const { activeFiles, setActiveFiles, activeComposerState } =
    useComposerContext();
  ...
  const { socketUrl } = useComposerContext();
  useEffect(() => {
    if (isVisible) {
  ...
  const shouldShowButtons =
    !activeComposerState ||
    !activeComposerState.threadId ||
    activeComposerState.threadId === threadId;
  ...
  function sendToExternalService(
    ...
    socket.on("connect", () => {
      if (data.type === "AVATAR_TALK") {
        socket.emit("AVATAR_TALK", data);
        console.log("Sent AVATAR_TALK event with data:", data);
      } else {
        socket.emit("AVATAR_TALK", data);
        console.log("Sent message event with data:", data);
      }
      socket.disconnect();
    });
  ...
  useEffect(() => {
    ...
    socket.on("DATA_IN", (data: { type: string; text: string }) => {
      console.log("Received DATA_IN from socket:", data);
      onChatSubmitted(data.text, [], undefined);
    });

    socket.on(
      "message",
      (data: { type: string; text: string; timestamp: number }) => {
        if (data.type === "DATA_IN") {
          console.log("Received message from another client:", data);
          onChatSubmitted(data.text, [], undefined);
        }
      }
    );
  ...
  return (
    <div className="pt-3 flex flex-col items-stretch" ref={ref}>
      ...
      <div className={inputContainerClass}>
        ...
        />
        <textarea
          placeholder={activeFiles.length === 0 ? "Ask me anything" : ""}
          value={inputValue}
          onChange={(e) => {
            handleInputChange(e);
