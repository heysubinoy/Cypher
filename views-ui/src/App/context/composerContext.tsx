...
interface ComposerContextType {
  ...
  setActiveComposerState: React.Dispatch<
    React.SetStateAction<ComposerState | undefined>
  >;
...
export const ComposerProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const [activeComposerState, setActiveComposerState] = useState<
    ...
    switch (event) {
      case "composer-message": {
        ...
        setComposerStates((states) => {
          ...
          setInputTokens(
            (currentTotal) => currentTotal + mergedState.inputTokens
          );
          setOutputTokens(
            (currentTotal) => currentTotal + mergedState.outputTokens
          );
      ...
      case "composer-error": {
        ...
        setComposerStates((states) => {
          ...
          const updatedStates = [...states];
          updatedStates[stateIndex] = mergeState(
      ...
      case "composer-done": {
        ...
        if (
          ...
          activeThreadRef.current.id === state.threadId
        ) {
          setActiveComposerState((prev) => ({
            title: state.title,
  ...
  const handleResponse = (event: MessageEvent<AppMessage>) => {
    ...
    switch (command) {
      ...
      case "init": {
        ...
        const stateThreads = threads.map(
          (state) =>
            ({
              id: state.threadId,
              title: state.title,
        ...
        if (activeThreadId) {
          activeThreadRef.current =
            stateThreads.find((t) => t.id === activeThreadId) ?? null;
      ...
      case "thread-data": {
        ...
        if (
          !activeComposerState ||
          (activeComposerState &&
            activeComposerState.threadId === activeThreadRef.current?.id)
        ) {
  ...
  const sendComposerRequest = (
    ...
    setActiveComposerState((state) => {
      if (state) {
        state.messages.push(
          new UserMessage(crypto.randomUUID(), request.input, request.image)
        );
      } else {
        state = {
          messages: [
...
const mergeState = (prev: ComposerState, state: ComposerState) => {
  ...
  for (const message of state.messages) {
    ...
    const matchingMessage = prev?.messages.find(
      (m) => m.id === message.id && m.role === message.role
    );
    ...
    if (!matchingMessage) {
      ...
      if (matchingMessage.role === "tool") {
        (matchingMessage as ToolMessage).metadata = (
