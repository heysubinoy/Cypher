...
export class LSPClient {
  ...
  activate = async (
    ...
    client.onRequest("wingman/compose", async (params: ComposerResponse) => {
      ...
      const socketUri = vscode.Uri.parse("http://localhost:3001");
      const externalSocketUri = await vscode.env.asExternalUri(socketUri);
      await this.composerWebView?.postMessage({
        command: "socket-url",
        value: externalSocketUri.toString(),
      });
    });
    ...
    // Ensure CSP headers explicitly allow WebSocket and long polling connections to localhost
    const cspMetaTag = `<meta http-equiv="Content-Security-Policy" content="default-src 'self'; connect-src ws://localhost:3001 http://localhost:3001; img-src 'self' https:; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline';">`;

    // Inject the CSP meta tag into the webview
    if (this.composerWebView) {
      this.composerWebView.html = `
		<!DOCTYPE html>
		<html lang="en">
		<head>
			${cspMetaTag}
		...
		<body>
			<script>
				window.addEventListener('message', (event) => {
					if (event.data.command === 'inject-csp') {
						document.head.insertAdjacentHTML('beforeend', event.data.value);
					}
				});
			</script>
		</body>
  ...
  setComposerWebViewReference = async (webview: vscode.Webview) => {
